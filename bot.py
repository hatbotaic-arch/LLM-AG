...
# ID администраторов (получаем через /chatid в личке)
ADMIN_1_ID = int(os.getenv("ADMIN_1_ID", "0"))
ADMIN_2_ID = int(os.getenv("ADMIN_2_ID", "0"))
ALLOWED_ADMIN_IDS = {ADMIN_1_ID, ADMIN_2_ID}
# ============================
...
async def call_yandex_gpt(messages):
    """Отправляет историю в YandexGPT и возвращает ответ."""
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=payload, timeout=30) as resp:
                if resp.status != 200:
                    ...
                    logger.error(f"YandexGPT error {resp.status}: {error_text}")
...
# --- Обработка личных сообщений (всем) ---
...
# --- Обработка сообщений в группе/супергруппе (только для администраторов) ---
async def handle_group_message(update, context):
    ...
    # Проверяем, что автор в списке разрешённых администраторов
    if user.id not in ALLOWED_ADMIN_IDS:
        return
...
def main():
    if not TELEGRAM_BOT_TOKEN or not YANDEX_API_KEY or not YANDEX_FOLDER_ID:
        logger.error("Ошибка: не заданы переменные окружения (токен бота, API-ключ Yandex, Folder ID).")
        return
